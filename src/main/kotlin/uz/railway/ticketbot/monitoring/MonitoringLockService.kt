 ...
 * other instances (or the next tick, if processing outlives 10 minutes)
 * never pick the same row up twice. The lock is released as soon as this
 * method returns - the slow, per-subscription railway.uz calls happen
 * afterwards, outside any database transaction.
 */
@Service
class MonitoringLockService(
    ...
    fun claimDueSubscriptions(batchSize: Int): List<TicketSubscription> {
        ...
        val provisionalNext = now.plus(Duration.ofMinutes(10))
